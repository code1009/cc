...
static int* key_pool_alloc()
{
	int* key_pointer = (int*)_key_pool.allocator.alloc(&_key_pool.storage);
	if (key_pointer == NULL)
	{
...
static void add(void)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		{
			test_out << "key_pool_alloc() failed:" << test_tindex(i) << test_tendl;
			test_assert(0);
