...
/////////////////////////////////////////////////////////////////////////////
//===========================================================================
cc_api static uint32_t item_key(const item_t* item_pointer)
{
	uint32_t a = (item_pointer->key1 >> 8) & 0xff;
	uint32_t b = (item_pointer->key1) & 0xff;
	uint32_t c = (item_pointer->key2 >> 8) & 0xff;
	uint32_t d = (item_pointer->key2) & 0xff;

	return (c << 24) | (d << 16) | (a << 8) | (b);
}

cc_api static cc_hash_value_t item_key_hash(const void* pointer)
{
#if (1==cc_config_compiler_msvc)
#pragma warning(disable:4311) // warning C4311: '형식 캐스트': 'const void *'에서 'uint32_t'(으)로 포인터가 잘립니다.
#pragma warning(disable:4302) // '형식 캐스트': 'const void *'에서 'uint32_t'(으)로 잘립니다.
#endif
...
cc_api static bool item_key_equal(const void* left, const void* right)
{
#if (1==cc_config_compiler_msvc)
...
static bool items_initialize()
{
	...
	cc_unordered_map_initialize(
		&_items.container,
		item_key_hash, item_key_equal, _items.elements, item_max_count, sizeof(item_t));
...
static void add(void)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		key = item_key(item_pointer);
#if (1==cc_config_compiler_msvc)
#pragma warning(disable:4312) // '형식 캐스트': 'uint32_t'에서 더 큰 'void *'(으)로의 변환입니다.
#endif
		rv = cc_unordered_map_add(&_items.container, (void*)key, item_pointer);
#if (1==cc_config_compiler_msvc)
...
#endif
			hash_value = item_key_hash((void*)key);
#if (1==cc_config_compiler_msvc)
#pragma warning(default:4312)
#endif
			test_out
...
#pragma warning(disable:4312)
#endif
	index = cc_unordered_map_find(&_items.container, (void*)key);
#if (1==cc_config_compiler_msvc)
#pragma warning(default:4312)
#endif
	if (index != cc_invalid_index)
	{
		item_pointer = (item_t*)cc_unordered_map_element_second(&_items.container, index);
